...
if (isProd && !process.env.BOT_WEBHOOK_SECRET) {
    throw new Error('❌ BOT_WEBHOOK_SECRET is required in production');
}

// JWT secret - fallback only allowed in dev
const JWT_SECRET = new TextEncoder().encode(
    process.env.JWT_SECRET || (isProd ? '' : 'dev-secret-change-in-production')
);
